...
def plota_lambert(concentracoes, absorbancias, desv_pad, coefs, abs_prev = None):
    """
    ...
    plt.title('Concentração por absrobância tradicional (CoCl2))')
    plt.legend(loc='lower left', bbox_to_anchor=(1, 0), borderaxespad=0.5)
    plt.grid(True)
    plt.savefig(f"ajuste tradicional",bbox_inches='tight')
    plt.show()
    

def lei_lambert(concentracoes, absorbancias, desv_pad, abs_prev = None):
    """
    ...
    plota_lambert(concentracoes, absorbancias, desv_pad, coefs, abs_prev)
...
def mqm_fuzzy_2Dplot(x_data, y_data, solucao, x_real=None, prevw = 0, titulo_grafico = "Ajuste por mínimos quadrados fuzzy", xlabel = "x", ylabel = "y"):
    ...
    Args:
        ...
        x_label (str, optional): rótulo do eixo x (padrão: "Dados Reais").
        y_label (str, optional): rótulo do eixo y (padrão: "Dados Fuzzy").
...
#         for x in x_data:
    ...
    plt.grid()
    plt.xlabel("Eixo x")
    plt.ylabel("Eixo y - Dados Fuzzy Triangulares")
...
def mqm_fuzzy_3Dplot(x_data, y_data, solucao, x_real=None, prevw = 0, titulo_grafico = "Ajuste por mínimos quadrados fuzzy", xlabel = "x", ylabel = "y"):
...
#     ax.set_xticklabels(x_ticks)
    ...
    ax.set_ylabel('Eixo y - Dados Fuzzy Triangulares')
    ax.set_zlabel('Eixo z - Pertinência')
...
def prev_mqmi_fuzzy_2Dplot(x_data, y_data, x_data_prev, y_data_prev, xlabel = r"$X_{fuzzy}$", ylabel = r"$Y_{fuzzy}$"):
    ...
    for s in range(len(x_data_prev)):
        ...
        for i in range (len(x_data_prev[s])):
            y = [y_data_prev[i][0][1] for _ in range(len(x_data_prev[0][i][s]))]
            plt.plot(x_data_prev[0][i][s], y, color = cores[s], linewidth=1, alpha = 0.8)
            x_pico = x_data_prev[0][i][s][1+len(x_data_prev[0][i][s])//2]
            y_pico = y_data_prev[i][0][1]
            plt.scatter(x_pico, y_pico, color = cores[s], alpha = 0.8, s=5)
    ...
    plt.title(f"Previsões de {xlabel}")
    plt.legend(loc='lower left', bbox_to_anchor=(1, 0), borderaxespad=0.5)
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.grid()
...
def prev_mqmi_fuzzy_3Dplot(x_data, y_data, x_data_prev, y_data_prev, xlabel = r"$X_{fuzzy}$", ylabel = r"$Y_{fuzzy}$"):
    ...
    for s in range(len(x_data_prev)):
        ...
        for i in range (len(x_data_prev[s])):
            y = [y_data_prev[i][0][1] for _ in range(len(x_data_prev[0][i][s]))]
            z1 = np.linspace(0,1, 1+len(x_data_prev[0][i][s])//2) 
            z2 = np.linspace(1,0, len(x_data_prev[0][i][s])//2)
            z = np.concatenate((z1, z2))
            ax.plot(x_data_prev[0][i][s], y, z, color = cores[s], linewidth=1, alpha = 0.8)
            x_pico = x_data_prev[0][i][s][1+len(x_data_prev[0][i][s])//2]
            y_pico = y_data_prev[i][0][1]
...
def ajuste_fuzzy(x_data, y_data, x_real = None, prevw = 0, titulo_grafico = "Ajuste por mínimos quadrados fuzzy", x_label = "Dados Reais", y_label = "Dados Fuzzy"):
    ...
    Args:
        ...
        x_label (str, optional): rótulo do eixo x nos gráficos (padrão: "Dados Reais").
        y_label (str, optional): rótulo do eixo y nos gráficos (padrão: "Dados Fuzzy").
    ...
    mqm_fuzzy_2Dplot(x_ord, y_ord, solucoes, x_real_ord, prevw, titulo_grafico)
    mqm_fuzzy_3Dplot(x_ord, y_ord, solucoes, x_real_ord, prevw, titulo_grafico)
...
def lei_lambert_fuzzy(x_data, y_data, x_real = None, y_prev = None, titulo_grafico = "Ajuste por mínimos quadrados fuzzy", xlabel = "x", ylabel = "y"):
    ...
    Args:
        ...
        prevw (int, optional): número de pontos futuros a prever (padrão: 0).
        titulo_grafico (str, optional): título dos gráficos gerados (padrão: "Ajuste por mínimos quadrados fuzzy").
        xlabel (str, optional): rótulo do eixo x nos gráficos (padrão: "x").
        ylabel (str, optional): rótulo do eixo y nos gráficos (padrão: "y").
    ...
    mqm_fuzzy_2Dplot(x_ord, y_ord, solucoes, x_real_ord, titulo_grafico = titulo_grafico, xlabel = xlabel, ylabel = ylabel)
    mqm_fuzzy_3Dplot(x_ord, y_ord, solucoes, x_real_ord, titulo_grafico = titulo_grafico, xlabel = xlabel, ylabel = ylabel)
    plota_diametros(x_ord, y_ord, solucoes)
