...
def _tira_branco(media, desv_pad):
    ...
    Args:
        media (list): lista contendo a média de cada triplicata.
        desv_padr (list): lista contendo o desvio padrão de cada triplicata.
...
def _tira_branco_fuzzy(dados_fuzzy):
    ...
    for tupla in dados_fuzzy[1:]:
        for i in tupla:
            sub_branco = soma_interativa_0(i, oposto_branco)
...
def sistema_fuzzy_2x2(U, V):
    ...
    Returns:
        solucoes (array): lista contendo as diferentes matrizes solução com variáveis triangulares fuzzy encontradas.
...
def mqm_fuzzy (x_data, y_data):
    ...
    Returns:
        solucoes (array): lista contendo as soluções do sistema, com os coeficientes fuzzy representados como tuplas.
    ...
    for i in range (n):
        l1 = soma_interativa_0(V[0][0], V0[0][i])
        l2 = soma_interativa_0(V[1][0], V0[1][i])  
        V[0][0] = l1
        V[1][0] = l2
    ...
    solucoes = np.round(sistema_fuzzy_2x2(U, V),5)
...
#         for x in x_data:
            ...
            resultados.append(soma_interativa_0(prod, solucao[s][1]))
    ...
    for s in range (len(solucao)):
        ...
        for xp in x_prevw:
            ...
            resultsp.append(soma_interativa_0(prodp, solucao[s][1]))
...
def prev_mqmi_fuzzy(y_data, solucoes):
    ...
    Args:
        y_data (array-like): matriz coluna de dados fuzzy de saída, na forma de tuplas (a, b, c).
        solucoes (array): lista contendo as soluções do sistema, com os coeficientes fuzzy da banda ajustada representados como tuplas.
    ...
    for s in range(len(solucoes)):
        ...
        for i in range(len(y_data)):
            ...
            soma = soma_interativa_0(y_data[i][0], op_alpha2)
...
def calcula_diametros(y_data):
    """
    Calcula o diâmetro de um conjunto de dados fuzzy.
    
    Arg:
        y_data (array-like): vetor coluna de dados fuzzy (n x 1 x alpha).
    
    Return: 
        diametros (array): vetor coluna de diâmetros calculados para cada dado fuzzy.
    """
    diametros = np.zeros((len(y_data),1))
...
def calcula_fdiametros(x_data, solucoes):
    """
    Calcula o diâmetro ao longo de um ajuste linear fuzzy.
    
    Arg:
        x_data (array-like): vetor coluna de dados reais.
        solucoes: lista contendo as soluções do sistema, com os coeficientes fuzzy da banda ajustada representados como tuplas.
    
    Return: 
        diametros (array): vetor de diâmetros calculados para os dados fuzzy, em que cada linha representa uma solução.
    """
    ...
    for s in range(len(solucoes)):
        for i in range(len(x_data)):
            ...
            y = soma_interativa_0(prod, solucoes[s][1])
            diam = y[-1] - y[0]
            diametros[s][i] = diam
...
def plota_fdiametros(x_data, y_data, solucoes):
    """
    Calcula e plota os diâmetros ao longo de um ajuste linear fuzzy.
    ...
    diametros_s = calcula_fdiametros(x_data,solucoes)
...
def plota_diametros_prev(x_prev, y_data):
    ...
    for s in range(len(x_prev)):
        ...
        diam_xp = calcula_diametros(x_prev[s])
...
def media(lista):
    ...
    Arg: 
        lista (list): conjunto de dados.
    
    Return: 
        med (float): média calculada.
...
def metr_maximo(a, b):
    """ Calcula a métrica do máximo para um par de dados.
    
    Arg: 
        a, b (list): dados.
    
    Return: 
        mmax (float): métrica do méximo calculada.
...
def ajuste_fuzzy(x_data, y_data, x_real = None, prevw = 0, titulo_grafico = "Ajuste por mínimos quadrados fuzzy", xlabel = "Dados Reais", ylabel = "Dados Fuzzy"):
    ...
    plota_fdiametros(x_ord, y_ord, solucoes)
...
def lei_lambert_fuzzy(x_data, y_data, x_real = None, y_prev = None, titulo = "Curva de calibração fuzzy", xlabel = "Concentração", ylabel = "Absorbância"):
    ...
    Returns:
        solucoes (list): lista contendo as soluções do sistema, com os coeficientes fuzzy da banda ajustada representados como tuplas.
        x_prev (list of list, optional): lista contendo, para cada solução de mqm fuzzy, uma lista de vetores colunas (n x 1) representando os valores de x_prev em função dos alpha-níveis, em que cada vetor corresponde a um intervalo da função de pertinência por partes. 
   """
